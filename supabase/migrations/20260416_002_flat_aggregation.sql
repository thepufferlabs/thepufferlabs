-- ============================================================
-- Flat Aggregation Tables
-- ============================================================

-- 1. Latest value per country per indicator
...
AS $$
BEGIN
    -- Step 1: Build wb_country_latest
    TRUNCATE wb_country_latest;

    -- Get latest year per country+indicator, then join for values
    WITH latest_years AS (
        SELECT country_code, indicator_code, MAX(year) AS latest_year
        FROM wb_indicator_values
        WHERE value IS NOT NULL
        GROUP BY country_code, indicator_code
    ),
    latest_with_values AS (
        SELECT
            ly.country_code,
            ly.indicator_code,
            ly.latest_year,
        ...
        JOIN wb_indicator_values v
            ON v.country_code = ly.country_code
            AND v.indicator_code = ly.indicator_code
            AND v.year = ly.latest_year
    ),
    prev_years AS (
        SELECT
            lv.country_code,
            ...
            lv.latest_year,
            lv.latest_value,
            v2.value AS prev_value,
            v2.year AS prev_year
        FROM latest_with_values lv
        LEFT JOIN wb_indicator_values v2
            ON v2.country_code = lv.country_code
            AND v2.indicator_code = lv.indicator_code
            AND v2.year = lv.latest_year - 1
    ...
    SELECT
        p.country_code,
        COALESCE(c.name, p.country_code),
        c.region,
        c.income_level,
        p.indicator_code,
        COALESCE(i.name, p.indicator_code),
        p.latest_year,
        p.latest_value,
        p.prev_value,
        CASE WHEN p.prev_value IS NOT NULL AND p.prev_value != 0
             THEN ROUND(((p.latest_value - p.prev_value) / ABS(p.prev_value)) * 100, 2)
        END,
        s.min_val, s.max_val, s.avg_val, s.cnt, s.first_yr
    FROM prev_years p
    LEFT JOIN wb_countries c ON c.code = p.country_code
    LEFT JOIN wb_indicators i ON i.code = p.indicator_code
    LEFT JOIN stats s ON s.country_code = p.country_code AND s.indicator_code = p.indicator_code;
    ...
    FROM (
        ...
        FROM wb_country_latest WHERE latest_value IS NOT NULL
    ) sub
    WHERE l.country_code = sub.country_code AND l.indicator_code = sub.indicator_code;
    ...
    FROM (
        ...
        FROM wb_country_latest WHERE latest_value IS NOT NULL AND region IS NOT NULL
    ) sub
    WHERE l.country_code = sub.country_code AND l.indicator_code = sub.indicator_code;

    -- Step 2: Build wb_country_summary
    ...
    INSERT INTO wb_country_summary (
        ...
        trade_pct_gdp, fdi_net, internet_pct, electric_kwh, co2_per_capita,
        health_pct_gdp, edu_pct_gdp, latest_year, indicator_count
    )
    SELECT
        cl.country_code,
        MAX(cl.country_name),
        ...
        MAX(CASE WHEN cl.indicator_code = 'NY.GDP.MKTP.CD'    THEN cl.latest_year  END)::INT,
        ...
        MAX(cl.latest_year),
        COUNT(DISTINCT cl.indicator_code)::INT
    FROM wb_country_latest cl
    GROUP BY cl.country_code;

    -- Rankings
    UPDATE wb_country_summary s SET gdp_rank = sub.rnk
    FROM (SELECT country_code, RANK() OVER (ORDER BY gdp DESC NULLS LAST) AS rnk FROM wb_country_summary WHERE gdp IS NOT NULL) sub
    WHERE s.country_code = sub.country_code;

    UPDATE wb_country_summary s SET gdp_pc_rank = sub.rnk
    FROM (SELECT country_code, RANK() OVER (ORDER BY gdp_per_capita DESC NULLS LAST) AS rnk FROM wb_country_summary WHERE gdp_per_capita IS NOT NULL) sub
    WHERE s.country_code = sub.country_code;

    UPDATE wb_country_summary s SET pop_rank = sub.rnk
    FROM (SELECT country_code, RANK() OVER (ORDER BY population DESC NULLS LAST) AS rnk FROM wb_country_summary WHERE population IS NOT NULL) sub
    WHERE s.country_code = sub.country_code;
