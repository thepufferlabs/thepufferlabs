import { discoverAllDocs, fetchFileContent, extractToc, getAllDocFiles } from "@/lib/github";
...
export default async function DocPage({ params }: PageProps) {
    ...
    return (
      <div className="min-h-screen bg-navy flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-text-muted mb-4">The repository &ldquo;{repo}&rdquo; was not found or has no docs.</p>
          <Link href={`${basePath}/docs/`} className="text-teal hover:underline text-sm">
  ...
  return (
    <div className="min-h-screen bg-navy flex pt-16">
      ...
      <DocsSidebar registry={registry} currentRepo={repo} currentPath={filePath} />

      {/* Middle — content */}
      <main className="flex-1 min-w-0">
        {/* Breadcrumb */}
        <div className="sticky top-16 z-10 border-b border-[var(--theme-border)] bg-navy/90 backdrop-blur-md pl-10 pr-4 sm:px-8 lg:pl-8 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs text-text-dim font-mono">
              <Link href={`${basePath}/docs/`} className="hover:text-teal transition-colors">
                docs
              </Link>
              <span>/</span>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-8 py-6 sm:py-10">
          ...
          {config.topics.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-6">
              {config.topics.map((topic) => (
                <span key={topic} className="inline-block px-2 py-0.5 rounded-full text-[10px] font-mono bg-teal/5 text-teal/60 border border-teal/10">
                  {topic}
                </span>
              ))}
            </div>
          ...
          <div className="mt-16 pt-8 border-t border-[var(--theme-border)] flex items-center justify-between gap-4">
            {prevFile ? (
              <Link href={docHref(prevFile.path)} className="group flex flex-col items-start text-sm">
                <span className="text-[10px] text-text-dim uppercase tracking-wider mb-1">Previous</span>
                <span className="text-text-muted group-hover:text-teal transition-colors">&larr; {prevFile.title}</span>
              </Link>
            ) : (
              <div />
            )}
            {nextFile ? (
              <Link href={docHref(nextFile.path)} className="group flex flex-col items-end text-sm">
                <span className="text-[10px] text-text-dim uppercase tracking-wider mb-1">Next</span>
                <span className="text-text-muted group-hover:text-teal transition-colors">{nextFile.title} &rarr;</span>
              </Link>
            ) : (
              <div />
            )}
      ...
      {/* Right sidebar — TOC */}
      <div className="hidden xl:block border-l border-[var(--theme-border)]">
