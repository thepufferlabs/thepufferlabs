...
export default function CodeBlock({ className, children }: CodeBlockProps) {
    ...
    async function highlight() {
      try {
        ...
        const html = await codeToHtml(code, {
          lang,
          theme: theme === "dark" ? "night-owl" : "github-light",
        });
  ...
  return (
    <div className="group relative my-4 rounded-lg border border-[var(--theme-border)] bg-code-bg overflow-hidden">
      {/* Header bar */}
      <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--theme-border)] bg-[var(--theme-white-alpha-5)]">
        <span className="text-[10px] font-mono uppercase tracking-wider text-text-dim">{language || "text"}</span>
        <button onClick={handleCopy} className="text-[10px] font-mono text-text-dim hover:text-teal transition-colors px-2 py-0.5 rounded hover:bg-[var(--theme-white-alpha-5)]">
          {copied ? "Copied!" : "Copy"}
        </button>
      </div>
      ...
      {highlightedHtml ? (
        ...
        <pre className="p-4 overflow-x-auto text-sm leading-relaxed bg-code-bg">
